        ...
        match reg {
            ...
            Some(Regs::SFLU_SO_BUFF1) => {
                match self.current_cmd {
                    ...
                    Some(_) => {
                        warn!("W25Q: unknow read on {} (PC: {:x})", self.current_cmd.as_ref().unwrap(), mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap());
                        return 0;
                    },
                    None => {
                        warn!("W25Q: unknow read when none commad issued (PC: {:x})", mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap());
                        return 0;
                    }
                }
            },
            Some(_) => {
                trace!("SFLU: read from {} (PC: {:x})", reg.unwrap(), mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap());
                return 0;
            },
            None => {
                trace!("SFLU: read from unknow 0x{:x} (PC: {:x})", addr, mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap());
                return 0;
            }
        }
    }
            ...
            Some(Regs::SFLU_SI_BUFF) => {
                if let Some(cmd) = &self.current_cmd {
                    match cmd {
                        ...
                        _ => {warn!("W25Q: unknow write on INPUT BUFFER on {} (PC: {:x})", cmd, mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap());}
                    }
                ...
                match self.current_cmd {
                    ...
                    Some(_) => {
                        trace!("W25Q: cmd {} (PC: {:x})", self.current_cmd.as_ref().unwrap(), mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap());
                    },
                    None => {
                        warn!("W25Q: cmd unknow 0x{:x} (PC: {:x})", value, mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap());
                    }
                }
            },
            Some(_) => trace!("SFLU: write to {} = 0x{:x}  (PC: {:x})", reg.unwrap(), value, mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap()),
            None => trace!("SFLU: write to 0x{:x} = 0x{:x}  (PC: {:x})", addr, value, mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap()),
        }
    }
        ...
        if let Some(cmd) = &self.current_cmd {
            match cmd {
                Commands::PAGE_PROGRAM => {
                    ...
                    self.current_addr = self.data_buff[2] as u32 | ((self.data_buff[1] as u32) << 8) | ((self.data_buff[0] as u32) << 16);
                    trace!("W25Q: WRITE to chip {} addr 0x{:x} data: {:x?}", self.current_chip, self.current_addr, self.data_buff);

                    // Update ROM mem
                    if self.current_chip == 0 {
                        let pos = (self.current_addr as u64) + crate::mem_map::ROM1_START;
                        mc.mem_write(pos, &self.data_buff).expect(format!("Failed to write to memory 0x{:x}", pos).as_str());
                    }
...
impl Module for Sflu {
    fn load(self, mc: &mut Unicorn<()>) {
        ...
        mc.mmio_map(crate::mem_map::IO_SFLU3_START, crate::mem_map::IO_SFLU3_LENGTH, Some(move |mc: &mut Unicorn<()>, addr: u64, size: usize| {return self_ref.borrow_mut().sflu_read(mc, addr, size)}), Some(move |mc: &mut Unicorn<()>, addr: u64, size: usize, value: u64| {self_ref_clone.borrow_mut().sflu_write(mc, addr, size, value)})).expect("failed to map sflu");
    }
}