...
impl Serial {
    pub fn serial_read(mc: &mut Unicorn<()>, addr: u64, _size: usize) -> u64 {
        ...
        match reg {
            ...
            Some(_) => {
                trace!("serial: read from {} (PC: {:x})", reg.unwrap(), mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap());
                return 0;
            },
            None => {
                trace!("serial: read from unknow 0x{:x} (PC: {:x})", addr, mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap());
                return 0;
            }
        }
    }
    ...
    pub fn serial_write(mc: &mut Unicorn<()>, addr: u64, _size: usize, value: u64) {
        ...
        match reg {
            ...
            Some(_) => trace!("serial: write to {} = 0x{:x}  (PC: {:x})", reg.unwrap(), value, mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap()),
            None => trace!("serial: write to 0x{:x} = 0x{:x}  (PC: {:x})", addr, value, mc.reg_read(unicorn_engine::RegisterARM::PC).unwrap()),
        }
    }
}
...
impl Module for Serial {
    fn load(self, mc: &mut Unicorn<()>) {
        ...
        mc.mmio_map(crate::mem_map::IO_SERIAL1_START, crate::mem_map::IO_SERIAL1_LENGTH, Some(Self::serial_read), Some(Self::serial_write)).expect("failed to map serial1");
        mc.mmio_map(crate::mem_map::IO_SERIAL2_START, crate::mem_map::IO_SERIAL2_LENGTH, Some(Self::serial_read), Some(Self::serial_write)).expect("failed to map serial2");
