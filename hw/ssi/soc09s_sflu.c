...
static void sflu_read(void *opaque, hwaddr addr, uint64_t *val, unsigned size)
{
    Soc09sSflu *s = opaque;
    switch (addr) {
    case SFLU_STATE:
        *val = FIELD_DP32(val, SFLU_STATE, SFLU_BUSY, 0);
        break;
    case SFLU_CTRL1:
        *val = FIELD_DP32(val, SFLU_CTRL1, XCE_CHIP_0, s->selected_chip == 0);
        *val = FIELD_DP32(val, SFLU_CTRL1, XCE_CHIP_1, s->selected_chip == 1);
        break;
    case SFLU_CTRL2:
        *val = FIELD_DP32(val, SFLU_CTRL2, XCE_CHIP_2, s->selected_chip == 2);
        *val = FIELD_DP32(val, SFLU_CTRL2, XCE_CHIP_3, s->selected_chip == 3);
        break;
    case SFLU_SO_BUFF1:
...
static void sflu_write(void *opaque, hwaddr addr, uint64_t val, unsigned size)
{
    ...
    case SFLU_CTRL1:
        ...
        if (FIELD_EX32(val, SFLU_CTRL1, XCE_CHIP_0)) {
            s->selected_chip = 0;
        } else if (FIELD_EX32(val, SFLU_CTRL1, XCE_CHIP_1)) {
            s->selected_chip = 1;
        }
        break;
    case SFLU_CTRL2:
        ...
        if (FIELD_EX32(val, SFLU_CTRL2, XCE_CHIP_2)) {
            s->selected_chip = 2;
        } else if (FIELD_EX32(val, SFLU_CTRL2, XCE_CHIP_3)) {
            s->selected_chip = 3;
        }
        break;
    case SFLU_SI_BUFF:
...
static void soc09s_sflu_realize(DeviceState *dev, Error **errp)
{
    ...
    Soc09sSflu *s = SOC09SFLU(dev);

    memory_region_init_io(&s->iomem, OBJECT(s), &sflu_ops, s, "sflu", 0x1000);
